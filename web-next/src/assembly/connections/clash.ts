    ...
    const active = (raw.connections ?? []).map((conn) => {
      ...
      if (!pre) {
        ...
        connection.downloadSpeed =
          asClash(connection).download - asClash(pre).download
        connection.uploadSpeed =
          asClash(connection).upload - asClash(pre).upload
      }
...
const asClash = (connection: Connection) =>
  connection as ClashConnectionRawMessage

const getNetwork = (c: ClashConnectionRawMessage) => {
  if ('fdae' in c) return c.metadata.network
...
export const connectionAccessor: ConnectionAccessor = {
  ...
  rule: (connection) => {
    ...
    return clash.rulePayload
      ? `${clash.rule}: ${clash.rulePayload}`
      : clash.rule
  },
  ...
  networkType: (connection) => {
    ...
    return (
      connection.fdae?.network ||
      `${clash.metadata.type} | ${getNetwork(clash)}`
    )
  },
  ...
  host: (connection) => {
    ...
    if (connection.fdae)
      return connection.fdae.domain || clash.metadata.destinationIP
  ...
  process: (connection) => {
    ...
    return (
      metadata.process ||
      metadata.processPath?.replace(/^.*[/\\](.*)$/, '$1') ||
      '-'
    )
  },
  destination: (connection) => {
    ...
    if (
      getFinalProxyType(clash) === PROXY_TYPE.Direct &&
      clash.metadata.remoteDestination
    ) {
  ...
  isDirect: (connection) =>
    getFinalProxyType(asClash(connection)) === PROXY_TYPE.Direct,
  smartBlock: (connection) => asClash(connection).metadata.smartBlock,
}

export const getConnectionDisplayValue =
  createGetConnectionDisplayValue(connectionAccessor)
