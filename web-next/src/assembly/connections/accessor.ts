...
const getVisibleChains = (
  ...
  if (
    (options.mode === 'card' || !options.showFullProxyChain) &&
    chains.length > 2
  ) {
  ...
  (
    ...
    switch (key) {
      ...
      case CONNECTIONS_TABLE_ACCESSOR_KEY.Outbound:
        return connection.fdae?.outbound || accessor.chains(connection)[0] || ''
      case CONNECTIONS_TABLE_ACCESSOR_KEY.DlSpeed:
        if (connection.rateKnown === false) return '—'
      ...
      case CONNECTIONS_TABLE_ACCESSOR_KEY.SourceIP:
        return connection.fdae
          ? accessor.sourceIP(connection)
          : getIPLabelFromMap(accessor.sourceIP(connection))
      case CONNECTIONS_TABLE_ACCESSOR_KEY.Dialer:
        return connection.fdae?.dialer || '—'
      case CONNECTIONS_TABLE_ACCESSOR_KEY.MAC:
        return connection.fdae?.mac || '—'
      case CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort:
