...
export const fetchProxies = async () => {
  ...
  const sortIndex = fdaeState.groups.map((group) => group.name)
  ...
  proxyGroupList.value = Object.values(proxyData.proxies)
    .filter((proxy) => proxy.fdaePolicy !== undefined)
    .sort((prev, next) => {
...
const getProviderNameByProxy = (proxyName: string) => {
  ...
  if (hinted) {
    return proxyProviederList.value.some((provider) => provider.name === hinted)
      ? hinted
      : ''
  }
...
const latencyTestForSingle = async (
  ...
  if (IPv6test.value) {
    try {
      const { data: ipv6LatencyResult } = await fetchNodeLatency(
        now,
        IPV6_TEST_URL,
        2000,
      )
  ...
  return await fetchNodeLatency(
    independentLatencyTest.value ? proxyName : now,
    url,
    timeout,
  )
}
...
const untestableProxyTypes = new Set([
  PROXY_TYPE.Reject,
  PROXY_TYPE.RejectDrop,
  PROXY_TYPE.Block,
])
const isLatencyTestable = (name: string) => {
  const type = proxyMap.value[name]?.type.toLowerCase() as
...
export const proxyGroupLatencyTest = async (proxyGroupName: string) => {
  ...
  if (IPv6test.value) {
    try {
      ...
      all?.forEach((name) => {
        IPv6Map.value[getNowProxyNodeName(name)] =
          ipv6LatencyResult[name] > NOT_CONNECTED
      })
    } catch {
      all?.forEach((name) => {
