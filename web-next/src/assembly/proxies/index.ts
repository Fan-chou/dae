...
import { state as fdaeState, selectMember, probeGroup } from '@/fdae/client'
...
export const hiddenGroupMap = useStorage<Record<string, boolean>>(
...
export const getTestUrl = (groupName?: string) => {
  ...
  const groupTestUrl = groupTestUrls.value.find(
  ...
  const proxyNode =
    proxyMap.value[groupName] ||
    proxyProviederList.value.find((p) => p.name === groupName)
 ...
 */
export const latencyMapOf = (
  groupName?: MaybeRefOrGetter<string | undefined>,
) =>
  computed(() => {
    const group = fdaeState.groups.find(
      (item) => item.name === toValue(groupName),
    )
    return group
      ? new Map(
          group.members.map((member) => [
            member.name,
            member.alive ? (member.latency_ms ?? 0) : 0,
          ]),
        )
      : getLatencyMap('').value
  })
...
export const getLatencyByName = (proxyName: string, groupName?: string) => {
  return latencyMapOf(groupName).value.get(proxyName) ?? NOT_CONNECTED
}

export const getHistoryByName = (proxyName: string, groupName?: string) => {
  if (groupName && fdaeState.groups.some((group) => group.name === groupName))
    return [] // fdae has current measurements, no measurement history or timestamp.
  if (groupName && independentLatencyTest.value && can('independentLatency')) {
    const proxyNode = proxyMap.value[proxyName]
...
export const getNowProxyNodeName = (name: string) => {
  ...
  while (node.now && node.now !== node.name && !visited.has(node.name)) {
...
export const handlerProxySelect = async (
  proxyGroupName: string,
  proxyName: string,
) => {
  try {
    if (!proxyMap.value[proxyGroupName]?.selectable) return
    await selectMember(proxyGroupName, proxyName)
    await fetchProxies()
  } catch (e) {
...
// No per-node probe exists in the fdae API. Node latency remains read-only.
...
export const proxyGroupLatencyTest = (name: string) => {
  if (probing.has(name)) return probing.get(name)!
  const task = probeGroup(name)
    .then(() => fetchProxies())
    .catch(notifyRequestError)
    .finally(() => probing.delete(name))
  probing.set(name, task)
  return task
}
export const allProxiesLatencyTest = async () => {
  for (const name of proxyGroupList.value) await proxyGroupLatencyTest(name)
}
