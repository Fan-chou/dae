 ...
 */
export function getItemKeysByCategory(
  categoryKey: SETTINGS_MENU_KEY,
): string[] {
 ...
 */
export function getAllKeysForCategory(
  categoryKey: SETTINGS_MENU_KEY,
): string[] {
  const category = SETTINGS_CATEGORIES.find((c) => c.key === categoryKey)
  if (!category) return []
...
export const CONNECTIONS_ITEM_KEYS = keyMapByLabel(
  SETTINGS_MENU_KEY.connections,
)
...
function keyMapByLabel(categoryKey: SETTINGS_MENU_KEY): Record<string, string> {
  ...
  return Object.fromEntries(
    (category?.items ?? []).map((i) => [i.label, i.key]),
  )
}
