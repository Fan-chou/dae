import * as fdae from '@/fdae/client'
import { proxySnapshot } from '@/fdae/proxies'
...
export const fetchClashVersion = async () => ({
  data: await fdae.fetchStatus(),
})

export const fetchProxiesAPI = async () => ({ data: await proxySnapshot() })

export const selectProxyAPI = (group: string, name: string) =>
  fdae.selectMember(group, name)
...
export const fetchProxyGroupLatencyAPI = (
  ...
  return axios.get<Record<string, number>>(
    `/group/${encodeURIComponent(proxyName)}/delay`,
    {
      params: {
        url,
        timeout,
...
export const fetchProxyProviderAPI = async () => ({
  data: { providers: {} as Record<string, ProxyProvider> },
})
...
export const fetchRuleProvidersAPI = () => {
  return axios.get<{ providers: Record<string, RuleProvider> }>(
    '/providers/rules',
  )
}
...
export const getConfigsAPI = async (): Promise<{ data: Config }> =>
  fdae.unsupported()
...
export const createClashWebSocket = <T>(
  ...
  const resurl = new URL(
    `${getUrlFromBackend(backend).replace('http', 'ws')}/${url}`,
  )
  ...
  websocket.onmessage =
    url === 'logs' ? messageHandler : debounce(messageHandler, 100)
...
export const probeClashChannel = async (
  ...
  try {
    const res = await fetch(`${getUrlFromBackend(backend)}/v1/status`, {
      method: 'GET',
      headers: {
...
export const reloadConfigsAPI = async () => {
  const result = await fdae.reload()
  if (!result.queued) throw new Error('热重载未入队')
  return result
}
