<template>
  ...
  >
    ...
    >
      ...
      <div ref="columnsRef" class="flex gap-3 p-3 md:pr-2">
        <VirtualColumn
        ...
        >
          <template v-slot="{ item }: { item: string }">
            <component :is="renderComponent" :name="item" />
          </template>
        </VirtualColumn>
      </div>
    </div>
...
import GroupStatus from '@/fdae/GroupStatus.vue'
...
const scrollStatus = useSessionStorage<
  Record<PROXY_TAB_TYPE, SavedScrollPosition>
...
const findAnchorElement = (item?: string) => {
  const renderedItems = columnsRef.value?.querySelectorAll<HTMLElement>(
    '[data-proxy-page-item]',
  )
  ...
  if (item !== undefined) {
    return (
      [...renderedItems].find(
        (element) => element.dataset.proxyPageItem === item,
      ) ?? null
    )
  }
  ...
  scrollStatus.value[tab] = anchor
    ? {
        ...
        offset:
          ...
          proxiesRef.value.getBoundingClientRect().top,
...
const restoreScrollPosition = (tab = proxiesTabShow.value) => {
  ...
  const fallbackTop = Math.max(
    0,
    Number(typeof saved === 'number' ? saved : saved?.scrollTop) || 0,
  )
  ...
  const restoreWhenReady = () => {
    ...
    if (typeof saved !== 'number' && saved?.item) {
      anchorColumn ??= columnRefs.value.find((column) =>
        column.scrollToItem(saved.item),
      )
      ...
      if (anchor) {
        const currentOffset =
          anchor.getBoundingClientRect().top -
          proxiesEl.getBoundingClientRect().top
...
const cardVariant = computed(
  () => `${cardType.value}:${displayTwoColumns.value ? 2 : 1}`,
)
const estimatedCardHeight = computed(() =>
  cardType.value === 'mobile' ? 88 : 112,
)

const foldersUiVisible = computed(
  () =>
    isProxyFolderModeActive.value &&
...
const columns = computed(() =>
  displayTwoColumns.value
    ? [
        filterContent(renderPageItems.value, 0),
        filterContent(renderPageItems.value, 1),
      ]
    : [renderPageItems.value],
)

useResizeObserver(proxiesRef, syncScrollMargin)
watch([foldersUiVisible, displayTwoColumns, isMiddleScreen], () =>
