...
const makeInitValue = (): HistoryPoint[] => {
  ...
  return new Array(savedPoints).fill(0).map((_, i) => {
    ...
    return {
      name: timestamp,
      value: [timestamp, 0] as [number, number],
      init: true,
    }
  })
}
...
export const initSatistic = () => {
  ...
  const unwatchMemory = watch(
    ...
    (data) => {
      ...
      connectionsHistory.value.push({
        value: [
          timestamp,
          (state.status?.active_connections ?? 0) +
            (state.status?.udp_sessions ?? 0),
        ],
        name: timestamp,
      })

      memoryHistory.value = memoryHistory.value.slice(-1 * savedPoints)
      connectionsHistory.value = connectionsHistory.value.slice(
        -1 * savedPoints,
      )
    },
  )
  ...
  const unwatchTraffic = watch(
    ...
    (data) => {
      ...
      downloadSpeedHistory.value = downloadSpeedHistory.value.slice(
        -1 * savedPoints,
      )
      uploadSpeedHistory.value = uploadSpeedHistory.value.slice(
        -1 * savedPoints,
      )
    },
  )
