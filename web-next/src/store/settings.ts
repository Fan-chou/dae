...
migrateLegacyStorageKey(
  'config/show-seleted-for-now-node',
  'config/show-selected-for-now-node',
)
migrateLegacyStorageKey(
  'config/use-connecticon-card',
  'config/use-connection-card',
)
migrateLegacyStorageKey(
  'config/connecticon-table-size',
  'config/connection-table-size',
)
...
const migrateLegacyConnectionDisplayStyle = () => {
  ...
  if (legacyValue === 'true' || legacyValue === 'false') {
    localStorage.setItem(
      nextKey,
      legacyValue === 'true'
        ? CONNECTION_DISPLAY_STYLE.CARD
        : CONNECTION_DISPLAY_STYLE.TABLE,
    )
  }
...
const replaceLegacyTheme = (theme: string, defaultTheme: string) => {
  ...
  if (
    [...ALL_THEME, ...customThemes.value.map((theme) => theme.name)].includes(
      theme,
    )
  ) {
    return theme
...
export const isSidebarCollapsedConfig = useStorage(
...
export const font = computed({
  get: () => {
    if (__FONT__ === 'none') return FONTS.SYSTEM_UI
...
export const disablePullToRefresh = useStorage(
  'config/disable-pull-to-refresh',
  true,
)
export const displayAllFeatures = useStorage(
  'config/display-all-features',
  false,
)
export const blurIntensity = useStorage('config/blur-intensity', 10)
export const scrollAnimationEffect = useStorage(
  'config/scroll-animation-effect',
  true,
)
export const IPInfoAPI = useStorage<IP_INFO_API>(
  'config/geoip-info-api',
  IP_INFO_API.IPSB,
)
...
export const autoDisconnectIdleUDP = useStorage(
  'config/auto-disconnect-idle-udp',
  false,
)
export const autoDisconnectIdleUDPTime = useStorage(
  'config/auto-disconnect-idle-udp-time',
  300,
)
export const keyboardShortcuts = useStorage<Record<string, string>>(
...
export const autoConnectionCheck = useStorage(
  'config/auto-connection-check',
  true,
)
...
export const overviewCardOrder = useStorage<
  { card: OVERVIEW_CARD; visible: boolean }[]
>('config/overview-card-order', defaultOverviewCardOrder)

// 确保所有卡片都在配置中。存量配置首次补入全球连接时放在连接拓扑前；
// 其他缺失卡片仍追加到末尾，已有全球连接的自定义顺序不改。
const allCardTypes = Object.values(OVERVIEW_CARD)
const existingCardTypes = new Set(
  overviewCardOrder.value.map((item) => item.card),
)
...
if (missingCards.length > 0) {
  ...
  for (const card of missingCards) {
    ...
    if (card === OVERVIEW_CARD.EarthGlobeCard) {
      const topologyIndex = nextOrder.findIndex(
        ({ card }) => card === OVERVIEW_CARD.TopologyCharts,
      )
      nextOrder.splice(
        topologyIndex === -1 ? nextOrder.length : topologyIndex,
        0,
        item,
      )
    } else {
...
export const earthIPInfoAPI = useStorage<IP_INFO_API>(
  'config/earth-ip-info-api',
  IP_INFO_API.IPIP,
)
export const earthVisualMode = useStorage<'flat' | 'space'>(
  'config/earth-visual-mode',
  'flat',
)
export const earthProjection = useStorage<'3d' | '2d'>(
  'config/earth-projection',
  '3d',
)
...
// proxies
export const collapseGroupMap = useStorage<Record<string, boolean>>(
...
export const displayFinalOutbound = useStorage(
  'config/show-selected-for-now-node',
  false,
)
...
export const independentLatencyTest = useStorage(
  'config/independent-latency-test',
  false,
)
export const speedtestTimeout = useStorage<number>(
...
export const automaticDisconnection = useStorage(
  'config/automatic-disconnection',
  false,
)
...
export const proxyPreviewType = useStorage(
  'config/proxy-preview-type',
  PROXY_PREVIEW_TYPE.AUTO,
)
export const hideUnavailableProxies = useStorage(
...
export const manageHiddenGroup = useStorage(
  'config/manage-hidden-group-mode',
  false,
)

export const displayGlobalByMode = useStorage(
  'config/display-global-by-mode',
  false,
)

export const proxyGroupIconSize = useStorage('config/proxy-group-icon-size', 24)
export const proxyGroupIconMargin = useStorage(
  'config/proxy-group-icon-margin',
  6,
)
...
export const groupProxiesByProvider = useStorage(
  'config/group-proxies-by-provider',
  false,
)
export const useSmartGroupSort = useStorage(
  'config/use-smart-group-sort',
  false,
)
...
export const showFullProxyChain = useStorage(
  'config/show-full-proxy-chain',
  true,
)
export const tableSize = useStorage<TABLE_SIZE>(
  'config/connection-table-size',
  TABLE_SIZE.LARGE,
)
export const tableWidthMode = useStorage(
  'config/table-width-mode',
  TABLE_WIDTH_MODE.AUTO,
)
export const connectionTableColumns = useStorage<
  CONNECTIONS_TABLE_ACCESSOR_KEY[]
>('config/connection-table-columns', [
  ...
  CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort,
...
export const connectionCardLines = useStorage<
  CONNECTIONS_TABLE_ACCESSOR_KEY[][]
>('config/connection-card-lines', DETAILED_CARD_STYLE)

// Upgrade existing browser layouts once; subsequent user customizations remain intact.
const cardEndpointFieldsVersion = useStorage(
  'config/fdae-card-endpoints-version',
  0,
)
if (cardEndpointFieldsVersion.value < 1) {
  const current = new Set(connectionCardLines.value.flat())
  const extra = [
    [
      CONNECTIONS_TABLE_ACCESSOR_KEY.SourceIP,
      CONNECTIONS_TABLE_ACCESSOR_KEY.SourcePort,
    ],
    [CONNECTIONS_TABLE_ACCESSOR_KEY.MAC],
    [
      CONNECTIONS_TABLE_ACCESSOR_KEY.Destination,
      CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort,
    ],
  ]
    .map((line) => line.filter((key) => !current.has(key)))
    .filter((line) => line.length)
  if (extra.length)
    connectionCardLines.value = [
      ...connectionCardLines.value.slice(0, 1),
      ...extra,
      ...connectionCardLines.value.slice(1),
    ]
  cardEndpointFieldsVersion.value = 1
}

export const sourceIPLabelList = useStorage<SourceIPLabel[]>(
  'config/source-ip-label-list',
  [],
)
export const resolveClientHostname = useStorage(
  'config/resolve-client-hostname',
  false,
)

// rules
export const displayNowNodeInRule = useStorage(
  'config/display-now-node-in-rule',
  true,
)
export const displayLatencyInRule = useStorage(
  'config/display-latency-in-rule',
  true,
)
export const disconnectOnRuleDisable = useStorage(
  'config/disconnect-on-rule-disable',
  true,
)
...
// logs
export const logRetentionLimit = useStorage<number>(
  'config/log-retention-limit',
  1000,
)
...
export const logSearchHistory = useStorage<string[]>(
