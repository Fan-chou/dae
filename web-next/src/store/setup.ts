...
if (!backendList.value.length) {
  try {
    const saved = JSON.parse(localStorage.getItem('kdae-ui-settings') || '{}')
    if (saved.baseUrl && typeof saved.secret === 'string') {
      const url = new URL(saved.baseUrl, location.origin)
        ...
        backendList.value = [
          {
            ...
            secondaryPath: url.pathname.replace(/\/$/, ''),
...
if (
  activeUuid.value &&
  !backendList.value.some((item) => item.uuid === activeUuid.value)
) {
  activeUuid.value = ''
}
...
export const openBackendManager = (
  view: BackendManagerView = { mode: 'list' },
) => {
...
export const switchActiveBackend = (direction: 1 | -1) => {
  ...
  const currentIndex = backendList.value.findIndex(
    (backend) => backend.uuid === activeUuid.value,
  )
  const startIndex = currentIndex >= 0 ? currentIndex : 0
  const nextIndex =
    (startIndex + direction + backendList.value.length) %
    backendList.value.length
