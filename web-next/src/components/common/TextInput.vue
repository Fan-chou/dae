<template>
  <label ref="anchorRef" :class="['input input-sm', { 'pe-1': clearable }]">
    <input
      v-model="inputValue"
      ref="inputRef"
      type="text"
      ...
      :aria-activedescendant="
        isMenuOpen && activeMenuIndex >= 0
          ? optionId(activeMenuIndex)
          : undefined
      "
    ...
    <button
      ...
      type="button"
      class="btn btn-ghost btn-circle btn-xs h-5 min-h-5 w-5 shrink-0 p-0"
      :aria-label="'清除' + (ariaLabel || placeholder || '输入')"
      @click="clearInput"
    >
      <XMarkIcon class="h-3 w-3" />
    </button>

    <Teleport v-if="isMounted" to="#app-content">
        ...
        >
          ...
          >
            <span class="min-w-0 flex-1 truncate"
              ><slot name="menu" :item="item">{{ item }}</slot></span
            >
            <CheckIcon
...
const { close, panelStyle, remeasure } = useFloatingMenu(
  anchorRef,
  menuRef,
  isMenuOpen,
  {
    maximumHeight: 256,
    minimumWidth: props.menuMinWidth,
  },
)
