<template>
  <div v-if="hasVisibleItems" class="text-sm">
    <template v-if="isVisibleBackendSwitch">
      <div class="settings-section-label">
        {{ $t('settingsSectionCurrentBackend') }}
      </div>
      <div class="settings-grid">
        <SettingItem :setting-key="k.backend" class="py-3">
    ...
    <template v-if="hasVisibleActions">
      <div class="settings-section-label">
        {{ $t('settingsSectionCoreOperations') }}
      </div>
      <div class="settings-grid">
        <div class="setting-item">
          <div class="setting-item-label">
            fdae 配置<span class="block mt-1 text-xs text-base-content/50"
              >global、routing 与独立路由规则</span
            >
          </div>
          <RouterLink to="/rules" class="btn btn-sm">编辑配置</RouterLink>
        </div>
        <SettingItem
        ...
        >
          ...
          <button
            :class="[
              'btn btn-sm min-w-11',
              action.key === k.upgradeCore && 'btn-neutral',
            ]"
    ...
    <template v-if="hasVisibleNetworkSettings">
      <div class="settings-section-label">
        {{ $t('settingsSectionNetworkListening') }}
      </div>
      <div class="settings-grid">
        <SettingItem :setting-key="k.ports" class="py-3">
          <div class="flex w-full flex-col gap-3">
        ...
        <SettingItem :setting-key="k.allowLan" :when="!!configs">
    ...
    <template v-if="hasVisibleUpgradeSettings">
      ...
      <div class="settings-grid">
        ...
        >
          ...
          <input v-model="autoUpgradeCore" class="toggle" type="checkbox" />
        </SettingItem>
      </div>
    </template>
    ...
    <template v-if="showDnsQuery">
      <div class="settings-section-label">
        {{ $t('settingsSectionDiagnostics') }}
      </div>
      <div class="settings-grid">
        <SettingItem :setting-key="k.DNSQuery" class="py-3">
          <DnsQuery />
        </SettingItem>
      </div>
    </template>
...
const hasVisibleUpgradeSettings = computed(
  () =>
    ...
    (isVisibleCheckUpgrade.value ||
      (checkUpgradeCore.value && isVisibleAutoUpgrade.value)),
)
