<template>
  <div class="flex flex-col gap-3 p-4">
    <div class="flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="flex items-center gap-2">
        <button
          class="btn btn-sm"
          @click="
            ((connectionCardLines = SIMPLE_CARD_STYLE), setRestOfColumns())
          "
        >
          {{ $t('simpleCardPreset') }}
        </button>
        <button
          class="btn btn-sm"
          @click="
            ((connectionCardLines = DETAILED_CARD_STYLE), setRestOfColumns())
          "
        >
    ...
    <div class="flex flex-col gap-2 overflow-hidden rounded-lg">
      ...
      >
        <button class="btn btn-sm gap-1" @click="addLine">
    ...
    <div class="flex flex-col gap-2">
      <div
        class="text-base-content/60 flex items-center justify-between px-1 text-xs"
      >
        <span>{{ $t('availableLabel') }}</span>
        <span class="badge badge-ghost badge-sm">{{
          restOfColumns.length
        }}</span>
      </div>
      <Draggable
...
const setRestOfColumns = () => {
  restOfColumns.value = Object.values(CONNECTIONS_TABLE_ACCESSOR_KEY)
    .filter(
      (key) =>
        !['close', 'process', 'geoip', 'remoteAddress', 'inboundUser'].includes(
          key,
        ),
    )
    .filter((key) => !connectionCardLines.value.flat().includes(key))
}
...
const removeField = (index: number, key: CONNECTIONS_TABLE_ACCESSOR_KEY) => {
  connectionCardLines.value[index] = connectionCardLines.value[index].filter(
    (k) => k !== key,
  )
