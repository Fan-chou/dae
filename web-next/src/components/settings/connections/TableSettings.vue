...
const restOfColumns = ref(
  Object.values(CONNECTIONS_TABLE_ACCESSOR_KEY)
    .filter(
      (key) =>
    ...
    .filter((key) => !connectionTableColumns.value.includes(key)),
)
...
const removeColumn = (key: CONNECTIONS_TABLE_ACCESSOR_KEY) => {
  connectionTableColumns.value = connectionTableColumns.value.filter(
    (k) => k !== key,
  )
