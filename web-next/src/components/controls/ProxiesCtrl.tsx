...
export default defineComponent({
  ...
  setup() {
    ...
    const modeList = computed(() => {
      return (
        configs.value?.['mode-list'] || configs.value?.['modes'] || defaultModes
      )
    })
    const needTranslateModes = computed(() => {
      return every(modeList.value, (mode) =>
        defaultModes.includes(mode.toLowerCase()),
      )
    })
    ...
    const hasNotCollapsed = computed(() => {
      return renderProxiesPageItems.value.some(
        (name) => collapseGroupMap.value[name],
      )
    })

    const handlerClickToggleCollapse = () => {
      collapseGroupMap.value = Object.fromEntries(
        renderProxiesPageItems.value.map((name) => [
          name,
    ...
    return () => {
      const tabs = (
        <SegmentedControl
          ...
          onUpdate:modelValue={(value) =>
            (proxiesTabShow.value = value as PROXY_TAB_TYPE)
          }
      ...
      const upgradeAllIcon = proxiesTabShow.value ===
      ...
      const modeSelect = false && configs.value && (
        <SelectInput
          class={[
            'select select-sm',
            isLargeCtrlsBar.value ? 'min-w-40' : 'min-w-24',
          ]}
      ...
      const sort = (
        <SelectInput
          ...
          onUpdate:modelValue={(value) =>
            (proxySortType.value = value as PROXY_SORT_TYPE)
          }
      ...
      const searchInput = (
        <div
          class={['relative w-32 flex-1', isLargeCtrlsBar.value && 'max-w-80']}
        >
          <button
