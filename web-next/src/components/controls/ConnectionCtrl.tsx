...
export default defineComponent({
  ...
  setup() {
    ...
    const { isLargeCtrlsBar } = useCtrlsBar(() =>
      isConnectionCard.value ? 860 : 720,
    )
    ...
    const sortDirectionLabel = () =>
      t(
        SORT_DIRECTION_LABEL_KEY[
          SORT_TYPE_VALUE_KIND[connectionSortType.value]
    ...
    return () => {
      const sortForCards = (
        <div class={`join flex-1 ${isLargeCtrlsBar.value ? 'min-w-46' : ''}`}>
          ...
          <button
            ...
            onMouseenter={(e) =>
              showTip(e, sortDirectionLabel(), { appendTo: 'parent' })
            }
          >
            {connectionSortDirection.value === SORT_DIRECTION.ASC ? (
        ...
        <SelectInput
          ...
          onUpdate:modelValue={(value) =>
            (connectionCardGroupKey.value =
              value as ConnectionGroupableKey | null)
          }
          options={[
      ...
      const toggleGroupsButton =
        isConnectionCard.value && connectionCardGroupKey.value !== null ? (
          <button
            ...
            onMouseenter={(e) =>
              showTip(e, toggleGroupsLabel(), { appendTo: 'parent' })
            }
          >
            {hasExpandedConnectionCardGroups.value ? (
      ...
      const content = !isLargeCtrlsBar.value ? (
        <div class="flex flex-wrap items-center gap-2 p-2">
          ...
          <div class="join w-full">{searchInput}</div>
        </div>
      ) : (
      ...
      return <CtrlsBar inline>{content}</CtrlsBar>
    }
  },
})
