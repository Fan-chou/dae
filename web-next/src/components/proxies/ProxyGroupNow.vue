<template>
  <div class="flex min-w-0 flex-1 items-center gap-1 overflow-hidden">
    <span
      v-if="proxyGroup.fdaeAutomatic && !proxyGroup.now"
      class="truncate text-xs text-base-content/60"
      title="自动策略可能按目标站点选择不同节点，连接页显示实际出站"
      >自动选择</span
    >
    <template v-if="proxyGroup.now">
      ...
      <ProxyName
        ...
        :class="
          isNowAGroup &&
          'hover:bg-base-300 hover:-mx-1 hover:rounded-lg hover:px-1 hover:shadow'
        "
        class="text-base-content min-w-0 shrink! overflow-hidden whitespace-nowrap text-ellipsis text-xs md:text-sm"
    ...
    <template
      v-else-if="proxyGroup.type.toLowerCase() === PROXY_TYPE.LoadBalance"
    >
