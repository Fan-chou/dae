<template>
  ...
  >
    <Transition name="latency-state">
      ...
      ></span>
      <span v-else-if="readonly && state === 'empty'" class="latency-state"
        >—</span
      >
      <BoltIcon
      ...
      />
      <div v-else ref="latencyRef" class="latency-state tabular-nums">
        {{ latency }}
      </div>
    </Transition>
  </div>
...
const latency = computed(() =>
  getLatencyByName(props.name ?? '', props.groupName),
)
