<template>
  <div class="relative flex h-18 shrink-0 flex-col justify-between">
    ...
    >
      <span
        class="shrink-0 tracking-wider whitespace-nowrap uppercase tabular-nums"
      >
        {{ proxyGroup.fdaePolicy }} · {{ proxiesCount }}
      </span>
      <ProxyGroupFilter v-if="displayContent" :group-name="name" />
    </div>
    <div class="flex min-w-0 items-center gap-1">
      ...
      >
        <GroupTraffic :name="name" />
      </div>
      <LatencyTag
        :class="twMerge('bg-base-200/40 hover:bg-base-200/70 z-10 shrink-0')"
        :loading="isLatencyTesting"
        role="button"
        tabindex="0"
        aria-label="检测代理组"
        title="检测代理组"
        @keydown.enter.stop="emit('latency-test')"
        :name="proxyGroup.now"
...
import GroupTraffic from '@/fdae/GroupTraffic.vue'
import {
...
const visibilityToggleTip = computed(() => {
  ...
  return shortcut
    ? `${title}\n${t('manageHiddenGroupShortcutTip', { shortcut })}`
    : title
})
