  ...
  <div class="charts-card base-container w-full p-4">
    ...
    <div class="charts-card-grid grid grid-cols-2 gap-3">
      ...
      >
        ...
        <div
          class="text-base-content/50 flex items-center justify-between gap-2 text-xs"
        >
...
const connectionCount = computed(() =>
  state.status
    ? (state.status.active_connections ?? 0) + (state.status.udp_sessions ?? 0)
    : null,
)
const memoryStr = computed(() =>
  prettyBytesHelper(memory.value, { binary: true }),
)
...
const speedTooltipFormatter = (value: ChartTooltipParam[]) => {
  return value
    .map((item) =>
      formatHistoryTooltipParam(item, { binary: false, suffix: '/s' }),
    )
    .join('')
}
...
const connTooltipFormatter = (value: ChartTooltipParam[]) => {
  return value
    .map((item) => formatTimeSeriesTooltipParam(item, String))
    .join('\n')
}
</script>
