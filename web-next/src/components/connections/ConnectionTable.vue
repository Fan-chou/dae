...
const isManualTable = computed(
  () => tableWidthMode.value === TABLE_WIDTH_MODE.MANUAL,
)
const { t } = useI18n()
...
const columnDefinitions: ColumnDef<Connection>[] = [
  ...
  },
  {
    ...
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.Type),
  ...
  },
  {
    ...
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.Host),
  ...
  },
  {
    ...
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.Rule),
  ...
  },
  {
    ...
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.Chains),
    cell: ({ row }) => {
      const chains: VNode[] = []
      const isReverse =
  ...
  },
  {
    ...
    sortingFn: (prev, next) =>
      getConnectionDownload(prev.original) -
      getConnectionDownload(next.original),
  },
  {
    ...
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.Upload),
  ...
  {
    header: () => t(CONNECTIONS_TABLE_ACCESSOR_KEY.Dialer),
    id: CONNECTIONS_TABLE_ACCESSOR_KEY.Dialer,
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.Dialer),
    cell: highlightedCell(CONNECTIONS_TABLE_ACCESSOR_KEY.Dialer),
  },
  {
    header: () => t(CONNECTIONS_TABLE_ACCESSOR_KEY.MAC),
    id: CONNECTIONS_TABLE_ACCESSOR_KEY.MAC,
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.MAC),
    cell: highlightedCell(CONNECTIONS_TABLE_ACCESSOR_KEY.MAC),
  },
  {
    header: () => t(CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort),
    id: CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort,
    accessorFn: (original) =>
      getTableDisplayValue(
        original,
        CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort,
      ),
    cell: highlightedCell(CONNECTIONS_TABLE_ACCESSOR_KEY.DestinationPort),
  },
  {
  ...
  },
  {
    ...
    accessorFn: (original) =>
      getTableDisplayValue(original, CONNECTIONS_TABLE_ACCESSOR_KEY.GeoIP),
...
const columns: ColumnDef<Connection>[] = columnDefinitions.map((column) => ({
  ...
  enableGrouping:
    typeof column.id === 'string' && groupableKeySet.has(column.id),
}))
...
const columnPinning = useStorage<ColumnPinningState>(
  'config/table-column-pinning',
  {
    left: [],
    right: [],
  },
...
const tanstackTable = useVueTable({
  ...
  state: {
    ...
    get columnVisibility() {
      return {
        ...Object.fromEntries(
          Object.values(CONNECTIONS_TABLE_ACCESSOR_KEY).map((key) => [
            key,
            false,
          ]),
        ),
  ...
  onColumnSizingChange: (updater) => {
    if (isFunction(updater)) {
      ...
      columnWidthMap.value = updater as Record<
        CONNECTIONS_TABLE_ACCESSOR_KEY,
        number
      >
    }
...
const handleMouseMove = (e: MouseEvent) => {
  ...
  if (
    !isDragging.value &&
    Math.pow(deltaX, 2) + Math.pow(deltaY, 2) > DRAG_THRESHOLD
  ) {
