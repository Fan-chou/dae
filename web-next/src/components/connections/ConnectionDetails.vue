    ...
    <div class="flex h-[70dvh] max-h-[70dvh] flex-col overflow-hidden">
      ...
      >
        <template v-for="section in sections" :key="section.id">
          <div
            class="border-base-content/8 bg-base-200/40 rounded-lg border p-3"
          >
            <div class="text-primary mb-2 text-sm font-semibold">
              {{ section.title }}
            </div>
          ...
          >
            ...
            <div class="mt-1 flex flex-wrap">
              <div class="mr-3 flex items-center gap-1" v-if="details?.country">
                ...
                <template
                  v-if="details?.city && details?.city !== details?.country"
                >
                  {{ details?.city }},
                </template>
                <template
                  v-else-if="
                    details?.region && details?.region !== details?.country
                  "
                >
                  {{ details?.region }},
                </template>
      ...
      <!-- 原始 JSON -->
      <div v-if="activeTab === 'raw'" class="flex-1 overflow-y-auto p-4">
        <VueJsonPretty :data="infoConn?.fdae || infoConn">
          <template #renderNodeValue="{ node, defaultValue }">
            <template
              v-if="
                (node.path.startsWith('root.chains') ||
                  node.path.startsWith('root.chainList')) &&
...
import SegmentedControl, {
  type SegmentOption,
} from '@/components/common/SegmentedControl.vue'
...
const sourceIP = computed(() =>
  infoConn.value ? getConnectionSourceIP(infoConn.value) : '',
)
...
const sectionDefs: { id: string; keys: CONNECTIONS_TABLE_ACCESSOR_KEY[] }[] = [
  ...
  { id: 'outbound', keys: [KEY.Chains, KEY.Outbound, KEY.Dialer] },
]
...
const sections = computed(() => {
  ...
  return sectionDefs
    .map((def) => {
      ...
      if (def.id === 'basic') {
        rows.unshift({ label: 'ID', value: conn.fdae?.id || conn.id })
      }
