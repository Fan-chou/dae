...
type reloadWorker struct {
	...
	routingEpochHandoffEnabled bool
	adminHTTP                  *adminServer
...
func (w *reloadWorker) run() {
	...
	for req := range w.reloadManager.reloadReqs {
		...
		stagedHotHandoff := shouldUseStagedHotHandoff(w.routingEpochHandoffEnabled, freshDatapathReload, w.listener != nil)
		...
		if !stagedHotHandoff && !freshDatapathReload {
			newC.InjectBpf(obj)
			if w.c != nil {
				newC.InheritLpmIndices(w.c.EjectLpmIndices())
			}
		}
		...
		if w.adminHTTP != nil {
			w.adminHTTP.refresh(newConf.Global.AdminListen, newConf.Global.AdminSecret)
		}
