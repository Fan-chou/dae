...
PNPM=${PNPM:-pnpm}
KDAE_UI_DIR=${KDAE_UI_DIR:-"$ROOT/web-next"}
...
(
	...
	FONT=${FONT:-none} "$PNPM" build
)
ui_data=$(mktemp -d)
mkdir -p "$ui_data/www/kdae-ui/assets" "$ui_data/www/cgi-bin"
cp -a "$KDAE_UI_DIR/dist/." "$ui_data/www/kdae-ui/"
...
if ! grep -Eq '(\./|/kdae-ui/)assets/[^" ]+\.js' "$ui_data/www/kdae-ui/index.html"; then
