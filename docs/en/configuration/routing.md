...
conditions + a later selector-only `direct`/`block` fallback) and, when enabled,
...
`dial_mode != ip`). This is opt-in in fdae: set `auto_sniff_punt: true`
in the global block to enable it; the default is `false`.
