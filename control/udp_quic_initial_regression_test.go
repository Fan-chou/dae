...
// TestQuicInitial_FlowsThroughConvoyAndReadFrom end-to-end regression-tests
// the production QUIC ingress path after the experimental UDP dispatchers were
// retired:
//
//  1. QUIC Initial submitted via submitOrderedUDPIngress (convoy task pool).
//  2. The convoy runs the task, which calls handlePkt.
//  3. handlePkt dials the proxy and creates a UdpEndpoint whose replies are
//     read via ReadFrom (the simulation conn has no PacketReceiver).
//  4. Endpoint close completes without hanging.
//
// Live proxy replies are not injected here: the handler writes through anyfrom
// and needs a real netns. Reply FIFO and Close drain are covered by
// udp_endpoint_receiver_test.go.
func TestQuicInitial_FlowsThroughConvoyAndReadFrom(t *testing.T) {
	defer setupQuicInitialRegressionTestState(t)()
	...
	task := func() {
		if err := cp.handlePkt(nil, payload, src, dst, routingResult, flowDecision, false); err != nil {
			t.Errorf("handlePkt through convoy: %v", err)
		}
	}
	if !cp.submitOrderedUDPIngress(flowDecision.Key, udpTaskFunc(task)) {
	...
	if got := underlay.calls.Load(); got != 1 {
		t.Fatalf("proxy DialContext calls = %d, want 1 (convoy did not deliver task to handlePkt)", got)
	}
	...
	ue, ok := DefaultUdpEndpointPool.Get(key)
	if !ok || ue == nil {
		t.Fatal("expected QUIC endpoint to exist after handlePkt through convoy")
	}
	...
	case <-time.After(3 * time.Second):
		t.Fatal("UdpEndpoint.Close hung after convoy+ReadFrom QUIC path")
	}
}
