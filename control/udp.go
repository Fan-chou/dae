...
func (c *ControlPlane) handlePktOwned(lConn *net.UDPConn, data []byte, src, realDst netip.AddrPort, routingResult *bpfRoutingResult, flowDecision UdpFlowDecision, skipSniffing bool, prefetched *UdpEndpoint, prefetchKey UdpEndpointKey, prefetchOK bool) (err error) {
	...
	ueKey, ue, ueExists = lookupUdpEndpointForFlowWithPrefetch(flowDecision, routeScope, forceSymmetricKey, realDst, quicIdentified, prefetched, prefetchKey, prefetchOK)
	if ueExists {
...
func lookupUdpEndpointForFlow(flowDecision UdpFlowDecision, routeScope udpEndpointRouteScope, forceSymmetric bool, realDst netip.AddrPort, identified bool) (UdpEndpointKey, *UdpEndpoint, bool) {
	return lookupUdpEndpointForFlowWithPrefetch(flowDecision, routeScope, forceSymmetric, realDst, identified, nil, UdpEndpointKey{}, false)
}

func lookupUdpEndpointForFlowWithPrefetch(flowDecision UdpFlowDecision, routeScope udpEndpointRouteScope, forceSymmetric bool, realDst netip.AddrPort, identified bool, prefetched *UdpEndpoint, prefetchKey UdpEndpointKey, prefetchOK bool) (UdpEndpointKey, *UdpEndpoint, bool) {
	// Reuse ingress prefetch and repeated probes without skipping the local
	// destination-scoped precedence required for off-port QUIC.
	var keys [3]UdpEndpointKey
	var values [3]*UdpEndpoint
	var hits [3]bool
	count := 0
	get := func(key UdpEndpointKey) (*UdpEndpoint, bool) {
		if prefetchOK && key == prefetchKey && prefetched != nil && !prefetched.dead.Load() {
			return prefetched, true
		}
		for i := 0; i < count; i++ {
			if keys[i] == key {
				return values[i], hits[i]
			}
		}
		value, ok := DefaultUdpEndpointPool.Get(key)
		if count < len(keys) {
			keys[count], values[count], hits[count] = key, value, ok
			count++
		}
		return value, ok
	}
	...
	if !forceSymmetric {
		...
		if candidate, exists := get(symmetricKey); exists {
			...
			if srcOnlyKey != ueKey {
				if candidate, exists := get(srcOnlyKey); exists &&
...
func (c *ControlPlane) handlePkt(lConn *net.UDPConn, data []byte, src, realDst netip.AddrPort, routingResult *bpfRoutingResult, flowDecision UdpFlowDecision, skipSniffing bool) error {
