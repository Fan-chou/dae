...
//	dns_fast_path                         0       0
//	cache_hit_fullcone                    3       2   reuse cache hit; retain destination probe
//	cache_hit_sniff_eligible_cross_probe  3       3   cross-probe kept
//	cache_miss_fallback_hit               3       2   fallback key == first Get
//	binding_miss_existing_fullcone        3       3   no prefetch; retain destination probe
...
func TestUdpIngressTaskPoolGetCountByScenario(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			want: 2, // reused src-only hit plus fdae destination-scoped QUIC probe
		...
		},
		{
			...
			// handlePkt first Get plus fdae destination-scoped QUIC probe. owner Get is skipped because there is no
