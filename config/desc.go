...
There is no auto-switch to another node or group. false restores the historical behavior of forwarding QUIC on whatever node the group already selected.
A routing rule such as l4proto(udp) && dport(443) -> block is a separate, stricter kernel silent drop of all UDP/443 (including datagram nodes).`,
	"dial_mode": `Optional values of dial_mode are:
1. "ip". Dial proxy using the IP from DNS directly. This allows your ipv4, ipv6 to choose the optimal path respectively, and makes the IP version requested by the application meet expectations. For example, if you use curl -4 ip.sb, you will request IPv4 via proxy and get a IPv4 echo. And curl -6 ip.sb will request IPv6. This may solve some weird full-cone problem if your are be your node support that. TCP sniffing timeout is 0 in this mode.
UDP is different: if a name is already known (FakeIP look-back or a previously sniffed QUIC SNI), a node with resolve_dns pins that name to an IP; otherwise the datagram is sent as domain:port so the proxy can resolve. Only when no name exists does UDP fall back to the packet IP.
...
4. "domain++". Based on domain+ mode but force to re-route traffic using sniffed domain to partially recover domain based traffic split ability. It doesn't work for direct traffic and consumes more CPU resources.`,
	...
	"pprof_port":                   "Non-zero enables Go pprof on localhost:<port> (typical 6060). 0 disables. The listener is loopback-only and is not the LAN admin API.",
