...
type Global struct {
	...
	// BlockQuic, when true (default), REJECT-NO-DROP identified QUIC unless
	// the selected node's final chain is unreliable datagram UDP. false keeps
	// the historical "forward QUIC on whatever node was selected" behavior.
	BlockQuic             bool   `mapstructure:"block_quic" default:"true"`
